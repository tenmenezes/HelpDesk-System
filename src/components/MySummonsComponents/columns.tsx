...
export const createColumns = (
  ...
  // Verifica permissões via cookie de sessão (não usa localStorage)
  // O middleware já garante que só usuários autenticados chegam aqui,
  // então verificamos o tipo via fetch síncrono ou via um valor de contexto.
  // Como ColumnDef não é um hook, usamos a abordagem de verificar um data-attribute
  // ou simplesmente retornamos true para deixar o ChamadoForm controlar via useAuth.
  const canEditStatusAndPriority = () => {
    // A permissão real é verificada em SummonForm (useAuth hook)
    // Aqui retornamos true para exibir os campos; o backend bloqueia se não tiver permissão
    return true;
  };
  ...
  return [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        const canEdit = canEditStatusAndPriority();

        if (!canEdit) {
          // Apenas exibe o status sem permitir edição
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        if (!canEdit) {
          // Apenas exibe a prioridade sem permitir edição
